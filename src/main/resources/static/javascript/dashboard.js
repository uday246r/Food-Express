document.addEventListener("DOMContentLoaded", () => {
	...
	async function handleMenuItemFormSubmit(event) {
		...
		const price = document.querySelector('input#price').value.trim();
		const stock = document.querySelector('input#stock').value.trim();
		const description = document.querySelector('textarea#description').value.trim();
		const images = document.querySelector('input#images').files;
		const restaurantId = document.querySelector('input#restaurantId').value.trim();

		// Log collected values
		console.log("Collected values:", { name, price, stock, description, images, restaurantId });
	...
	// Cache order items to avoid refetching on every filter change
	...
	async function fetchOrdersByStatus(status) {
		...
		try {
			// Fetch data once and reuse for subsequent filter changes
			if (!cachedOrderItems) {
				const response = await fetch(url);
				if (!response.ok) {
					throw new Error(`HTTP error! Status: ${response.status}`);
				}
				cachedOrderItems = await response.json();
			}

			const data = cachedOrderItems;
			...
			if (status == "waiting") {
				...
				document.querySelectorAll('.accept-button').forEach(button => {
					button.addEventListener('click', function () {
						...
						.then(message => {
							row.remove(); // Remove the row from the table
							// Invalidate cache so next filter change fetches fresh data
	...
	function initGoogleMap(location) {
	  ...
	  // Fallback to a default view (India) if location is missing or invalid
	  let position = { lat: 20.5937, lng: 78.9629 };
	  let hasValidLocation = false;

	  if (location) {
		const parts = location.split(',').map(coord => parseFloat(coord.trim()));
		if (parts.length === 2 && !isNaN(parts[0]) && !isNaN(parts[1])) {
		  position = { lat: parts[0], lng: parts[1] };
		  hasValidLocation = true;
		} else {
		  console.warn("Invalid location data, showing default map:", location);
		}
	  } else {
		console.warn("No location set for restaurant, showing default map.");
	  }

	  // Initialize the map
	  const map = new google.maps.Map(mapElement, {
		center: position,
		zoom: hasValidLocation ? 15 : 5,
	  });

	  // Add a marker only when we have a valid restaurant location
	  if (hasValidLocation) {
		new google.maps.Marker({
		  position,
		  map,
		  title: `Location: ${position.lat}, ${position.lng}`,
		  icon: {
